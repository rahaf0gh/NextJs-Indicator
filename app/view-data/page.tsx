...
import { getData } from './dataFetcher'; // استيراد دالة الجلب من ملف dataFetcher
...
export default function ViewDataPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fetchedData = await getData(url, format); // هنا الجلب كما كان
        setData(fetchedData);
      } catch (error) {
  ...
  return (
    <div className='p-7'>
      ...
      <p className='text-center text-red-800 text-md'> قد يختلف تصميم الجدول عن الملف الأساسي لاختلاف مواضع الأعمدة في المتصفح</p>
