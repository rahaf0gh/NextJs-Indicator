...
export default function ChartPage() {
  ...
  const totalItems = data.length;  // إجمالي المنشآت
  const activityGates = new Set(data.map((item) => item[xKey])).size;  // عدد أبواب النشاط (القيم الفريدة في xKey)
  const sections = extraKey ? new Set(data.map((item) => item[extraKey])).size : 0;  // عدد الأقسام (القيم الفريدة في extraKey)
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
      ...
      {/* الرسم البياني الثالث (إذا توفر مفتاح إضافي) */}
      {extraKey && (
        <div className="bg-white p-4 rounded-xl shadow">
